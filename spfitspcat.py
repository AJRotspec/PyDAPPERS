...
class Transition:
    ...
    def fromcatline(cls, catline):
        ...
        alphanumeric = {cha: 10 * (ord(cha) - 65) + 100 for cha in 'ABCDEFGHI'}
        alphanumeric[' '] = 0
        def catint(string):
            # print(string)
            if string.isdigit():
        ...
        # try:
        j1 = [catint(catline[j:j + 2]) for j in range(55, 60, 2)]
        j2 = [catint(catline[j:j + 2]) for j in range(67, 72, 2)]
        # except: print(catline)
        if catline[62] in [str(i) for i in range(10)]:
            j1 += [catint(catline[61:63])]
            j2 += [catint(catline[73:75])]
...
class CatFile:
    ...
    #             return int(string)
    #         return alphanumeric[string[0]] + int(string[1])
    #     tomake['j1'] = tuple(catint(catline[j:j + 2]) for j in range(55, 60, 2))
    #     tomake['j2'] = tuple(catint(catline[j:j + 2]) for j in range(67, 72, 2))
    #     tomake['progT'] = progify(tomake['j1'], tomake['j2'])
    #     tomake['pred'] = float(catline[3:13])
    #     tomake['inten'] = float(catline[22:28])
    #     return tomake
